...
export function Navbar() {
  ...
  return (
    <nav className="fixed top-0 left-0 right-0 z-[1000] border-b border-[#e6e0d6] bg-[#fffdf8]/95 backdrop-blur-sm">
        ...
        <div className="flex justify-between h-16">
          ...
          <div className="flex md:hidden items-center">
            <button 
              ...
              className="rounded-full p-2 text-[#68665f] transition-colors hover:bg-[#f7f4ed] hover:text-[#171b17] focus:outline-none"
            >
      ...
      {isOpen && (
        <div className="absolute w-full border-b border-[#e6e0d6] bg-[#fffdf8] pb-4 md:hidden">
          <div className="px-2 pt-2 space-y-1 flex flex-col">
            <NavLink 
              to="/" 
              onClick={() => setIsOpen(false)}
              className={({ isActive }) => 
                `block rounded-xl px-3 py-2 text-sm font-medium ${isActive ? 'bg-[#f7f4ed] text-[#171b17]' : 'text-[#68665f] hover:bg-[#f7f4ed]'}`
              }
            >
